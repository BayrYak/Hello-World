...
Z = matrix(Z, BURNIN+N)
Z_v = rep(NA,(BURNIN+N)*(T+2))		
Z_v = matrix(Z_v, BURNIN+N)
...
#Hyperparameters for Z_y (The jump term)
...
#Hyperparameters for Z_v (The jump term)
musstar_v = rep(NA,(BURNIN+N)*(T+2))	
musstar_v = matrix(musstar_v, BURNIN+N)
...
epsilons[1,] = (Y[2:(T+1)] - mu[1]*deltat - Z[1,2:(T+1)]*B[1,2:(T+1)])/sqrt(V[1,1:T]*deltat)
epsilonv[1,] = (V[1,2:(T+1)] - kappa[1]*theta[1]*deltat - (1 - kappa[1]*deltat) * V[1,1:T] - Z_v[1,2:(T+1)]*B[1,2:(T+1)])/
  sqrt(V[1,1:T]*deltat)
...
#---------------------
  mustara[i] = sum((omega[i-1] + psi[i-1]^2) *(Y[2:(T+1)] - Z[i-1,2:(T+1)]*B[i-1,2:(T+1)])/(omega[i-1]*V[i-1,1:T]))
  
  mustarb[i] = sum(psi[i-1]*(V[i-1,2:(T+1)] - kappa[i-1]*theta[i-1]*deltat - 
                               (1 - kappa[i-1]*deltat)*V[i-1,1:T] -  Z[i-1,2:(T+1)]*B[i-1,2:(T+1)])/(omega[i-1]*V[i-1,1:T]))
...
#----------------------
  ...
  eps = (Y[2:(T+1)] - mu[i]*deltat - Z[1,2:(T+1)]*B[1,2:(T+1)])/sqrt(V[i-1,1:T]*deltat)
  
  epv = (V[i-1,2:(T+1)] - kappa[i-1]*theta[i-1]*deltat - (1 - kappa[i-1]*deltat) * V[i-1,1:T] - Z_v[1,2:(T+1)]*B[1,2:(T+1)])/
  ...
  thetastara[i] = sum(kappa[i-1]*(V[i-1,2:(T+1)] - (1 - kappa[i-1]*deltat)*V[i-1,1:T] - Z_v[i-1,2:(T+1)]*B[i-1,2:(T+1)])/
                        (omega[i]*V[i-1,1:T]))
  
  thetastarb[i] = sum(psi[i]*(Y[2:(T+1)] - mu[i]*deltat + (1/2)*V[i-1,1:T]*deltat - Z[i-1,2:(T+1)]*B[i-1,2:(T+1)])*kappa[i-1]/
                        (omega[i]*V[i-1,1:T]))
...
#-----------------------------
  ...
  kappastara[i] = sum(((theta[i] - V[i-1,1:T])*(V[i-1,2:(T+1)]-V[i-1,1:T]) + Z_v[i-1,2:(T+1)]*B[i-1,2:(T+1)]*(V[i-1,1:T]- theta[i]))/
                        (omega[i]*V[i-1,1:T]))
  
  kappastarb[i] = sum(psi[i]*(Y[2:(T+1)] - mu[i]*deltat - Z[i-1,2:(T+1)]*B[i-1,2:(T+1)])*(theta[i] - V[i-1,1:T])/
                        (omega[i]*V[i-1,1:T]))
  ...
  epsnew1 = (Y[2] - mu[i]*deltat  - Z[i,2]*B[i,2])/sqrt(V[i,1]*deltat)
  
  epsold1 = (Y[2] - mu[i]*deltat  - Z[i-1,2]*B[i-1,2] )/sqrt(V[i-1,1]*deltat)
  
  epvnew1 = (V[i,2] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i,1] - Z_v[i,2]*B[i,2])/
    sqrt(V[i,1]*deltat)
  # Внимание на V
  epvold1 = (V[i-1,2] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i-1,1] - Z_v[i-1,2]*B[i-1,2])/
    sqrt(V[i-1,1]*deltat)
  
  # Просто делить вероятностти не семприруем их из формулы, внимательно пробежаться
  ...
  for(j in 2:(T+1)){
    ...
    epsnew1 = (Y[(j+1)] - mu[i]*deltat - Z[i,j]*B[i,j])/sqrt(V[i,j]*deltat)
    
    epsold1 = (Y[(j+1)] - mu[i]*deltat - Z[i-1,j]*B[i-1,j])/sqrt(V[i-1,j]*deltat)
    
    epvnew1 = (V[i,(j+1)] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i,j] - Z_v[i,j]*B[i,j])/
      sqrt(V[i,j]*deltat)
    
   
    epvold1 = (V[i-1,(j+1)] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i-1,j]  - Z_v[i-1,j]*B[i-1,j])/
      sqrt(V[i-1,j]*deltat)
    
    #new - вперед по i  ; 0  --- back for j 
    #old - назад по i   ; 1  --- forward for j
    epsnew0 = (Y[j] - mu[i]*deltat - Z[i,(j-1)]*B[i,(j-1)] )/sqrt(V[i,(j-1)]*deltat)
    
    epsold0 = (Y[j] - mu[i]*deltat  - Z[i-1,(j-1)]*B[i-1,(j-1)])/sqrt(V[i-1,(j-1)]*deltat)
    
    epvnew0 = (V[i,j] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i,(j-1)]  - Z_v[i,j-1]*B[i,j-1])/
      sqrt(V[i,(j-1)]*deltat)
    
    epvold0 = (V[i-1,j] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i,(j-1) - Z_v[i,j-1]*B[i,j-1]])/
      sqrt(V[i,(j-1)]*deltat)
  ...
  epsnew0 = (Y[(T+2)] - mu[i]*deltat - Z[i,(T+1)]*B[i,(T+1)])/sqrt(V[i,(T+1)]*deltat)
  
  epsold0 = (Y[(T+2)] - mu[i]*deltat - Z[i,(T+1)]*B[i,(T+1)])/sqrt(V[i,(T+1)]*deltat)
  ...
  epvnew0 = (V[i,(T+2)] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i,(T+1)] - Z_v[i,(T+1)]*B[i,(T+1)])/
    sqrt(V[i,(T+1)]*deltat)
  # Знак смотреть 
  epvold0 = (V[i-1,(T+2)] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat) * V[i,(T+1)] - Z_v[i,(T+1)]*B[i,(T+1)])/
    sqrt(V[i,(T+1)]*deltat)
 ...
 #-------------------------- 
  # Размер прыжка сначала потом, моделируем индикаторы прыжка 
  muvol[i,1] = muv[i-1]
  Z_v[i,1] = rexp(1, muvol[i,1])
  #Correlation warning
  rho_j = rnorm(0,2)
  for (j in 2:(T+2)){			# update Z_volatiluty hyperparameters
    
    muva =  (V[i,j] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat)*V[i,j-1])/(omega[i]*V[i,j-1]*deltat)
    
    muvb = psi[i]*(Y[j] - mu[i]*deltat  - Z[i-1,(j-1)]*B[i-1,(j-1)])/(omega[i]*V[i,j-1]*deltat)
    
    musb = psi[i]*(V[i,j] - kappa[i]*theta[i]*deltat - (1 - kappa[i]*deltat)*V[i,j-1])/(omega[i]*V[i,j-1]*deltat)
    
    musstar[i,j] = (muva*B[i-1,j] - muvb*B[i-1,j] + (rho_j^2)*Z[i,j-1]*(Z[i-1,j]*mus[i-1])/sigma2s[i-1])/(((omega[i] + psi[i]^2)/(omega[i]*V[i,j-1]*deltat))*B[i-1,j] + 1/sigma2s[i-1])
    
    # Когда будешь подставлять для "B" для прыжков и нет, НЕ нужно добавлять Jump в exp(\mu), чтобы отсутствии прыжка было экспоненциальное распределение
    
    sigma2sstar[i,j] = 1/((1/(omega[i]*V[i,j-1]*deltat))*B[i-1,j] + (rho_j^2)/sigma2s[i-1])
...
#--------------------------
  ...
  for (j in 2:(T+2)){			# update Z hyperparameters
    ...
    sum(psi[i]*(Y[j] - mu[i]*deltat - Z[i-1,2:(T+1)]*B[i-1,2:(T+1)])*(theta[i] - V[i,j-1])/
          (omega[i]*V[i,j-1]))
...
mean(mu[1001:length(mu)])
sd(mu[1001:length(mu)])
